...
def return_best_k_params(X = None, Y = None,
    ...
    training_values : if cv_splits is None, then number of training values to evaluate with
    testing_values : if cv_splits is None, then number of testing values to evaluate with (use None for all non-train values)
    num_tests : if cv_splits is None, then number of evaluations to perform (with random split of training_values - testing_values)
    tensor_training_portion : fraction/number of hyperparameter combinations for the tensor completion to train on (out of the entire hyperparameter space)
    ...
    if training_portion < 1: portion_of_combinations = training_portion
    else: portion_of_combinations = training_portion/total_cells
