...
from notebooks.utilities import utils
...
from notebooks.tensor_completion_models.Tensor_Train import *
...
def train_tensor_completion(model_type, 
    ...
    if (model_type == 'cpd'):
        ...
        model = train_NeAT(train_indices = train_indices, train_values = train_values, cfg = cfg, verbose = verbose)
    ...
    #     epoch_loss = 0
        for batch in dataloader:
            ...
            # elif (model_type == 'cpd.smooth'):
                ...
                loss = (rec - targets).pow(2).sum()
                
                if (model_type == 'cpd.smooth'):
